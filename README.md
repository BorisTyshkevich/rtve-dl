...
- regenerates `data/<slug>/index.html` at the end of each run with columns:
  - Picture
  - Episode metadata (title/description ES+RU, release date, size, links)
...
- `data/<slug>/index.html` is regenerated automatically
...
- `data/<series_slug>/` for final `.mkv`
  - `data/<series_slug>/index.html` for local episode listing
    - 2-column layout: picture + metadata block
    - metadata is enriched from cached RTVE catalog API (`tmp/<slug>/catalog_*.json`)
    - includes: `title_es`, `description_es`, `title_ru`, `description_ru`, `release_date`, size
    - `Name` link tries `vlc://` first; if blocked it falls back to JS-generated `.m3u`
    - no static `.m3u` files are generated by backend
  - `tmp/<series_slug>/index_meta_ru.json` stores RU translation cache for index metadata
