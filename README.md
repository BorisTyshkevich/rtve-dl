...
- regenerates `data/<slug>/index.html` at the end of each run with columns:
  - Name (JS-generated `.m3u` download for VLC)
  - Size
  - Datetime
  - Download
...
- `data/<series_slug>/` for final `.mkv`
  - `data/<series_slug>/index.html` for local episode listing
    - `Name` link uses browser JS to generate and download a tiny `.m3u` playlist
      containing an absolute HTTP URL for that episode `.mkv` (no static `.m3u` files are generated by backend)
