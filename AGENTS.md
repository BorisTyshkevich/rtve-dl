# AGENTS.md - Project Context for AI Assistance

This file provides context for Codex (or other AI assistants) when working on this codebase.

## Project Overview

**rtve-dl** is a Spanish video downloader for RTVE.es that produces language-learning MKV files with multiple subtitle tracks (Spanish, English, Russian). It uses AI (Codex or Codex) for translation and MLX Whisper ASR for speech recognition fallback.
...
3. Send as compact TSV to backend (Codex default, Codex optional)
4. Parse response, write `.output.jsonl` or `.nochunk.out.jsonl`
5. Resume from existing JSONL on retry

### Translation Modes
Two execution strategies:
- **No-chunk** (Codex default): Single request with full episode context
...
| `codex_batch.py` | Translation chunking framework (Codex + Codex backends) |
...
- Backend: Codex (default) or Codex CLI
- Chunk size: `--codex-chunk-cues` CLI option
- Model: `-m` / `--model` (auto-routes to `--Codex-model` or `--codex-model`)
...
- `codex` - AI translation CLI (optional if using Codex backend)
...
# Test RU refs prompt with Codex
tools/test_ru_refs_prompt.sh --backend Codex --model sonnet
